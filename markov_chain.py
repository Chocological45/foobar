...
'''
def standardize(m, absorbing_indexes, non_absorbing_indexes):
    """
    Compute the standard form of a given transition matrix, m
    """
    for row in m:
        print(row)
    ...
    for row in range(len(m)):
        standard_m += [[None] * len(m[0])]

    for i in absorbing_indexes:
        for j, cell in enumerate(m[i]):
            if j not in [x for x in absorbing_indexes]:
                standard_m[i - len(non_absorbing_indexes)][j - len(non_absorbing_indexes)] = cell
            else:
                print(i - len(non_absorbing_indexes), j + len(non_absorbing_indexes))
                standard_m[j + len(non_absorbing_indexes)][i - len(non_absorbing_indexes)] = cell

    for i in non_absorbing_indexes:
        for j, cell in enumerate(m[i]):
            if j in [x for x in non_absorbing_indexes]:
                standard_m[i + len(absorbing_indexes)][j + len(absorbing_indexes)] = cell

    print("")
    for row in standard_m:
        print(row)
